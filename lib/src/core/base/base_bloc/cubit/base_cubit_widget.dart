  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: <SingleChildWidget>[
        ...
        BlocProvider(
          create: (BuildContext context) => cubit..withContextHandler(contextActivity),
        ),
      ],
      child: BlocListener<ContextActivityBloc, ContextActivityState>(
        listener: (BuildContext context, ContextActivityState state) => state.contextActivityHandler != null
            ? state.contextActivityHandler!(context)
            : null,
        child: BlocConsumer<C, S>(
          listener: listener ?? (BuildContext context, s) {},
          builder: (BuildContext context, s) => builder(context, s, context.read<C>()),
          buildWhen: buildWhen,
        ),
      ),
    );
